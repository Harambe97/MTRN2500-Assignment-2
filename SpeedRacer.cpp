...
SpeedRacer::SpeedRacer() {
	...
	FrontLeftWheel = new Wheels(1, 0, -1, 0, 0.75, 1);
	...
	FrontRightWheel = new Wheels(1, 0, 1, 0, 0.75, 1);
	...
	BackLeftWheel = new Wheels(-1, 0, -1, 0, 0.75, 1);
	...
	BackRightWheel = new Wheels(-1, 0, 1, 0, 0.75, 1);
...
SpeedRacer::SpeedRacer(VehicleModel * RemoteVehicles) {
	...
	for (std::vector<ShapeInit>::iterator it = RemoteVehicles->shapes.begin(); it != RemoteVehicles->shapes.end(); it++) {
		if (it->type == RECTANGULAR_PRISM) {
			...
			Wheels * cyl = new Wheels(0, 0, 0, 0, 0, 0);
...
void SpeedRacer::draw() {
	...
	for (std::vector<Shape *>::iterator it = shapes.begin(); it != shapes.end(); it++) {
		...
		Wheels * cyl = dynamic_cast<Wheels *>(*it);
